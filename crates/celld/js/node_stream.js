...
// Duplex/Transform/PassThrough port; the bundle form is terser to embed, so
// it is vendored verbatim between the "esm.sh bundle" markers below, with
// the bundle's external imports (process, buffer, events, string_decoder,
// util helpers, node error classes) mapped onto Cells' own seams in the
// header. Comment-only lines were rewrapped to 80 columns; the single code
// change (Duplex.prototype.destroy, matching Workerd) is marked inline.
//
// Also defines stream/promises, stream/web (re-exporting the global web
// stream classes lazily, so importing node:stream alone does not force
// them), and stream/consumers.
//
// Injected lazily into the generated stub module, so an isolate that never
// imports node:stream pays nothing.
(() => {
  "use strict";
  const { Buffer } = globalThis; // lazy global; forces node_buffer
  const EE = process.getBuiltinModule("node:events");
  const nextTick = process.nextTick;
  const __process$ = process;
  const __buffer$ = { Buffer };

  // ---- node:string_decoder (subset the bundle reaches for) ----------------
  // utf8/utf16le decode statefully through TextDecoder; base64 keeps the
  // 3-byte remainder; hex/latin1/ascii are alignment-free.
  class StringDecoder {
    constructor(encoding = "utf8") {
      ...
      if (this.encoding === "utf8" || this.encoding === "utf16le") {
        this.__td = new TextDecoder(
          this.encoding === "utf8" ? "utf-8" : "utf-16le");
      } else if (this.encoding.startsWith("base64")) {
        this.__rem = null;
      }
    }
    write(buf) {
      ...
      if (this.__td) return this.__td.decode(u8, { stream: true });
      let bytes = u8;
      if (this.__rem !== undefined) {
        const joined = this.__rem
          ? Buffer.concat([this.__rem, u8]) : Buffer.from(u8);
        const keep = joined.length % 3;
        this.__rem = keep ? joined.slice(joined.length - keep) : null;
        bytes = joined.slice(0, joined.length - keep);
      }
      return Buffer.from(
      ...
      let out = buf !== undefined && buf.length ? this.write(buf) : "";
      if (this.__td) out += this.__td.decode();
      else if (this.__rem) {
        out += this.__rem.toString(this.encoding);
        this.__rem = null;
      }
      return out;
    }
    text(buf, offset) { return this.write(buf.slice(offset)); }
  }
  ...
  // Only used as a property key: `pipeline[promisify]` / `eos[promisify]`
  // must be the custom-promisify slot node:util's promisify honors.
  ...
  // ---- node error classes (internal/errors subset) ------------------------
  // Factories rather than classes: the bundle constructs some with `new` and
  // calls others bare, and a function returning the error object serves both.
  const E = (code, Base, message) => {
  ...
  const received = (value) => {
    ...
    if (typeof value === "object")
      return `. Received an instance of ${
        value.constructor?.name ?? "Object"}`;
    let repr = inspect(value);
    if (repr.length > 27) repr = `${repr.slice(0, 25)}...`;
    return `. Received type ${typeof value} (${repr})`;
  };
  const oneOf = (expected) =>
    (Array.isArray(expected) ? expected : [expected]).join(" or ");
  const ERR_INVALID_ARG_TYPE = E(
    "ERR_INVALID_ARG_TYPE", TypeError,
    (name, expected, actual) =>
      `The ${name.includes(".") ? `"${name}" property` :
        `"${name}" argument`} must be of type ${
        oneOf(expected)}${received(actual)}`);
  const ERR_INVALID_ARG_VALUE = E(
    "ERR_INVALID_ARG_VALUE", TypeError,
    (name, value, reason = "is invalid") =>
      `The argument '${name}' ${reason}. Received ${inspect(value)}`);
  const ERR_OUT_OF_RANGE = E(
    "ERR_OUT_OF_RANGE", RangeError,
    (name, range, actual) =>
      `The value of "${name}" is out of range. It must be ${range}. ` +
      `Received ${actual}`);
  const ERR_INVALID_RETURN_VALUE = E(
    "ERR_INVALID_RETURN_VALUE", TypeError,
    (expected, name, value) =>
      `Expected ${expected} to be returned from the "${name}" function` +
      ` but got ${typeof value}.`);
  const ERR_METHOD_NOT_IMPLEMENTED = E(
    "ERR_METHOD_NOT_IMPLEMENTED", Error,
    (name) => `The ${name} method is not implemented`);
  const ERR_MISSING_ARGS = E(
    "ERR_MISSING_ARGS", TypeError,
    (...args) => {
      const names = args.map((a) => `"${a}"`).join(" or ");
      return `The ${names} argument${args.length > 1 ? "s" : ""} ` +
        `must be specified`;
    });
  const ERR_MULTIPLE_CALLBACK =
    E("ERR_MULTIPLE_CALLBACK", Error, "Callback called multiple times");
  const ERR_SOCKET_BAD_PORT = E(
    "ERR_SOCKET_BAD_PORT", RangeError,
    (name, port, allowZero = true) =>
      `${name} should be >${allowZero ? "=" : ""} 0 and < 65536. ` +
      `Received ${port}.`);
  const ERR_STREAM_ALREADY_FINISHED = E(
    "ERR_STREAM_ALREADY_FINISHED", Error,
    (name) => `Cannot call ${name} after a stream was finished`);
  const ERR_STREAM_CANNOT_PIPE =
    E("ERR_STREAM_CANNOT_PIPE", Error, "Cannot pipe, not readable");
  const ERR_STREAM_DESTROYED = E(
    "ERR_STREAM_DESTROYED", Error,
    (name) => `Cannot call ${name} after a stream was destroyed`);
  const ERR_STREAM_NULL_VALUES = E(
    "ERR_STREAM_NULL_VALUES", TypeError,
    "May not write null values to stream");
  const ERR_STREAM_PREMATURE_CLOSE =
    E("ERR_STREAM_PREMATURE_CLOSE", Error, "Premature close");
  const ERR_STREAM_PUSH_AFTER_EOF =
    E("ERR_STREAM_PUSH_AFTER_EOF", Error, "stream.push() after EOF");
  const ERR_STREAM_UNSHIFT_AFTER_END_EVENT = E(
    "ERR_STREAM_UNSHIFT_AFTER_END_EVENT", Error,
    "stream.unshift() after end event");
  const ERR_STREAM_WRITE_AFTER_END =
    E("ERR_STREAM_WRITE_AFTER_END", Error, "write after end");
  const ERR_UNKNOWN_ENCODING = E(
    "ERR_UNKNOWN_ENCODING", TypeError,
    (enc) => `Unknown encoding: ${enc}`);
  const ERR_UNKNOWN_SIGNAL = E(
    "ERR_UNKNOWN_SIGNAL", TypeError,
    (signal) => `Unknown signal: ${signal}`);
  const ERR_ILLEGAL_CONSTRUCTOR =
    E("ERR_ILLEGAL_CONSTRUCTOR", TypeError, "Illegal constructor");
  function AbortError(message = "The operation was aborted", options) {
  ...
  /* esm.sh bundle: readable-stream@4.2.0, vendored verbatim */
    ...
    function endReadableNT(state, stream) {
      ...
      if (
        ...
        if (stream.writable && stream.allowHalfOpen === false) {
          ...
          const wState = stream._writableState;
          // We don't expect the writable to ever 'finish' if writable is
  ...
  "lib/internal/streams/writable.js"(exports, module) {
    ...
    function _write(stream, chunk, encoding, cb) {
      ...
      if (typeof encoding === "function") {
        cb = encoding;
        // Simulates nodejs/node commit
        // dbed0319ac438dcbd6e92483f3280b1dc6767e00
    ...
    function finish(stream, state) {
      ...
      if (state.autoDestroy) {
        const rState = stream._readableState;
        // We don't expect the readable to ever 'end' if readable is
        // explicitly set to false.
...
var require_duplex = __commonJS({
  "lib/internal/streams/duplex.js"(exports, module) {
    ...
    // Cells fix, matching Workerd's streams_duplex: the loop above keeps
    // Readable's bare destroyImpl.destroy, so a destroyed Duplex would
    // never error buffered write callbacks. Writable's override does.
    ...
    function Transform(options) {
      ...
      if (readableHighWaterMark === 0) {
        options = {
          ...
          // a "bug" where we check needDrain before calling _write and
          // not after.
...
var require_stream = __commonJS({
  ...
  // stream/web: the global web-stream classes, behind getters so importing
  // node:stream alone does not materialize the lazy byte-stream/compression
  // globals.
  const web = {};
  for (const name of [
  ...
  // stream/consumers over anything async-iterable (node streams and Cells'
  // web ReadableStream both are).
  const consumers = {
    ...
    async json(stream) { return JSON.parse(await consumers.text(stream)); },
    async text(stream) {
      let out = "";
      const decoder = new TextDecoder();
      for await (const chunk of stream) {
        out += typeof chunk === "string"
